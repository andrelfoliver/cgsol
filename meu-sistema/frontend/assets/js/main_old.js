...
function updateCounters() {
  ...
  allCards.forEach(card => {
    const coord = card.getAttribute('data-coordenacao');
    const tipo = card.getAttribute('data-tipo');

    if (!coord || !tipo) return;

    const key = `${coord.toLowerCase()}-${tipo.toLowerCase().replace(/\s+/g, '')}`;
    counts[key] = (counts[key] || 0) + 1;
  });

  Object.keys(counts).forEach(key => {
    const selector = `[onclick*="filterProjects('${key.split('-')[0]}', '${key.split('-')[1]}')"] span`;
